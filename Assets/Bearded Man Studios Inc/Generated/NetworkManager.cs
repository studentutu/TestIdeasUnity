...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] CubeCustomMoveNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is CubeCustomMoveNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (CubeCustomMoveNetworkObject.Length > 0 && CubeCustomMoveNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(CubeCustomMoveNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<CubeCustomMoveBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateCubeCustomMove instead, its shorter and easier to type out ;)")]
		public CubeCustomMoveBehavior InstantiateCubeCustomMoveNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeCustomMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeCustomMoveBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<CubeCustomMoveBehavior>().networkObject = (CubeCustomMoveNetworkObject)obj;
		...
		public CubeCustomMoveBehavior InstantiateCubeCustomMove(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(CubeCustomMoveNetworkObject[index]);
			var netBehavior = go.GetComponent<CubeCustomMoveBehavior>();
